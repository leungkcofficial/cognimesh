...
def load_txt(file_path, summarize_ratio=0.2):
    try:
        ...
        try:
            move_file(new_file_path, os.path.join(output_dir, os.path.basename(file_path))) # type: ignore
            logger.info(f"Moved the original file to the output directory")
        except Exception as e:
            logger.error(f"Error moving the original file to the output directory: {e}")
